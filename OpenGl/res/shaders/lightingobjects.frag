...
void main( )
{
    ...
    vec3 combine = CalcDirLight( dirLight, norm, viewDir );
    
    // Point lights
    for ( int i = 0; i < NUMBER_OF_POINT_LIGHTS; i++ )
    {
        combine += CalcPointLight( pointLights[i], norm, FragPos, viewDir );
    }
    
    // Spot light
    combine += CalcSpotLight( spotLight, norm, FragPos, viewDir );
    
    color = texture(texture_diffuse, TexCoords) * vec4( combine, 1.0 );
}
